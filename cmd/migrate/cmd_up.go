...
func NewMigrateUpCommand() *cobra.Command {
	...
	flags := cmd.Flags()
	flags.BoolVar(&opts.Bitcoin, "bitcoin", false, "Apply Bitcoin migrations")
	...
	flags.BoolVar(&opts.Runes, "runes", false, "Apply Runes migrations")
...
func migrateUpHandler(opts *migrateUpCmdOptions, _ *cobra.Command, args migrateUpCmdArgs) error {
	...
	cloneURLWithQuery := func(u *url.URL, newQuery url.Values) *url.URL {
		clone := *u
		query := clone.Query()
	...
	applyMigrations := func(module string, sourcePath string, migrationTable string) error {
		...
		if args.N == 0 {
			m.Log.Printf("Applying migrations...\n")
			err = m.Up()
		} else {
			m.Log.Printf("Applying %d migrations...\n", args.N)
			err = m.Steps(args.N)
		}
		if err != nil {
			if !errors.Is(err, migrate.ErrNoChange) {
				return errors.Wrapf(err, "failed to apply %s migrations", module)
			}
	...
	if opts.Bitcoin {
		if err := applyMigrations("Bitcoin", opts.BitcoinSource, "bitcoin_schema_migrations"); err != nil {
			return errors.WithStack(err)
		}
	}
	if opts.Runes {
		if err := applyMigrations("Runes", opts.RunesSource, "runes_schema_migrations"); err != nil {
			return errors.WithStack(err)
		}
	}
	return nil
