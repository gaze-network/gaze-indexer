...
const getRuneTransactionByHash = `-- name: GetRuneTransactionByHash :one
...
type GetRuneTransactionByHashRow struct {
	Hash                    string
	BlockHeight             int32
	Index                   int32
	Timestamp               pgtype.Timestamp
...
func (q *Queries) GetRuneTransactionByHash(ctx context.Context, hash string) (GetRuneTransactionByHashRow, error) {
	row := q.db.QueryRow(ctx, getRuneTransactionByHash, hash)
	var i GetRuneTransactionByHashRow
	err := row.Scan(
		&i.Hash,
		&i.BlockHeight,
		&i.Index,
